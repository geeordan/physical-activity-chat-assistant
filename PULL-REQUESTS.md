...
Fork the project [on GitHub](https://github.com/geeordan/physical-activity-chat-assistant) and clone your fork
locally.

```text
$ git clone git@github.com:username/physical-actvity-chat-assistant.git
$ cd physical-activity-chat-assistant
$ git remote add upstream https://github.com/geeordan/physical-activity-chat-assistant.git
$ git fetch upstream
```
...
PACA does not yet have a style guide.
...
Fixes: https://github.com/geeordan/physical-activity-chat-assistant/issues
Refs: http://eslint.org/docs/rules/space-in-parens.html
```

If you are new to contributing to physical-activity-chat-assistant, please try to do your best at
...
at least two physical-activity-chat-assistant Collaborators (one Collaborator approval is enough if the
...
All PACA (physical-activity-chat-assistant) contributors who choose to review and provide feedback on Pull
...
1. Does this change make sense for physical-activity-chat-assistant?
2. Does this change make physical-activity-chat-assistant better, even if only incrementally?
...
physical-activity-chat-assistant collaborator landing the Pull Request but they can also be an
...
a particular change that makes physical-activity-chat-assistant better, but the individual might just
not want to have anything to do with physical-activity-chat-assistant ever again. The goal is not just
having good code.
...
Any physical-activity-chat-assistant core Collaborator (any GitHub user with commit rights in the
`geeordan/physical-activity-chat-assistant` repository) is authorized to approve any other contributor's
...
### Accept that there are different opinions about what belongs in PACA

Opinions on this vary, even among the members of the Technical Steering
Committee.

One general rule of thumb is that if physical-activity-chat-assistant itself needs it (due to historic
or functional reasons), then it belongs in physical-activity-chat-assistant.
...
make physical-activity-chat-assistant better. These may or may not make sense to add, but as with all
...
PACA has always optimized for speed of execution. If a particular change
can be shown to make some part of physical-activity-chat-assistant faster, it's quite likely to be
...
That said, performance is not the only factor to consider. PACA also
...
to make sure that it works on the platforms that PACA
